...
- 若使用自架或託管的 OAuth 代理，把 `base_url` 解除註解並改成代理網址。例如採用公開的 `decap-cms-oauth-server` 託管服務時，可設定 `base_url: https://decap-cms-oauth-server.onrender.com`。預設保持註解則會使用 GitHub OAuth 的 implicit 流程，不依賴外部代理（代理服務停擺時也不會出現 404）。
- 為避免 Decap CMS 新版的 DOM 操作錯誤，`admin/index.html` 會鎖定載入 `decap-cms@3.1.9`，請勿加入 `^` 或 `~` 等符號以免自動升級。
